...
void ExplicitEuler::integrate(int step) {
    ...
    profiler.value2file("/time/rhs", time);
    profiler.value2file("/time/rhs_elapsed", time_elapsed);

    subDomainKeyTreeHandler->copy(To::host, true, false);
    profiler.vector2file("/general/ranges", subDomainKeyTreeHandler->h_range);

    boost::mpi::communicator comm;
    sumParticles = numParticlesLocal;
    all_reduce(comm, boost::mpi::inplace_t<integer*>(&sumParticles), 1, std::plus<integer>());
    profiler.value2file("/general/numParticles", sumParticles);

    profiler.value2file("/general/numParticlesLocal", numParticlesLocal);
