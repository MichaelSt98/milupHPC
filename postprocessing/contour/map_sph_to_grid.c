...
static void usage(char *name) 
{
	fprintf(stderr, "Usage %s [options]\n"
			...
			"\t-H, --hsml\t\t Smoothing length. (default: use particle's hsml)\n"
			"\t-b, --beautify\t\t Use a 30%% longer hsml. Works only if --hsml is not set.\n"
			...
			"File format is 'x y [z] v_x v_y [v_z] mass [rho] [e] [hsml] [noi] [material_type] [number_of_flaws] [number_of_activated_flaws] [local_strain] [damage] [p] [S_xx [S_xy S_xz]] [[flaw_1] ... [flaw_number_of_flaws]]'.\n"
			, name);
...
# error "No such dimension."
	...
	for (d = 0; d < DIM; d++) {
		...
		//printf("%e\n", hsml);
		ghsml[d] = (int) floor(hsml/gridlength[d]) + 1;
		//printf("ghsml[%d]: %d\n", d, ghsml[d]);
	}
...
#endif
    double buff;
	fscanf(disk.data, "%le", &p.hsml);
	p.hsml *= 3.;
	fscanf(disk.data, "%le", &buff); ////&p.noi);
	p.noi = 50;
	/* read in material type */
	fscanf(disk.data, "%le", &buff); //&p.material_type);
	p.material_type = 0;
#ifdef FRAGMENTATION
...
int check_particle(void) 
{
	...
	for (d = 0; d < DIM; d++) {
		...
		if (hsml == 0) {
			p.ghsml[d] = (int) floor(p.hsml/gridlength[d]) + 1;
		}
...
void rhs(void)
{
	...
	(k - ghsml[0]) >= 0 ? (kmin = k - ghsml[0]) : (kmin = 0);
	(k + ghsml[0]) <= (gridsize-1) ? (kmax = k + ghsml[0]) : (kmax = gridsize-1);
#if (DIM > 1)
	(l - ghsml[1]) >= 0 ? (lmin = l - ghsml[1]) : (lmin = 0);
	(l + ghsml[1]) <= (gridsize-1) ? (lmax = l + ghsml[1]) : (lmax = gridsize-1);
#endif
#if (DIM > 2)
	(m - ghsml[2]) >= 0 ? (mmin = m - ghsml[2]) : (mmin = 0);
	(m + ghsml[2]) <= (gridsize-1) ? (mmax = m + ghsml[2]) : (mmax = gridsize-1);
#endif
#if (DIM == 1)
